...
#构建Xception，输出倒数第二全连接层的特征向量，结构为（m,2048）
def create_base_network(input_shape):
    input = Input(shape=input_shape)
    model = xception.Xception(include_top=True, weights='imagenet', input_tensor=input, input_shape=input_shape, pooling=None, classes=1000)
